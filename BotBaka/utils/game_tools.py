...
from BotBaka.data.game_data import PropertyConverter, GameBaseInitData, PropNames
...
class GameTools:
    ...
    def calc_properties(level, _str, vit, agi):
        ...
        final_atk = GameBaseInitData.BASE_ATK + (level - 1) * LEVEL_TO_ATK + _str * STR_TO_ATK
        ...
        final_def = GameBaseInitData.BASE_DEF + (level - 1) * LEVEL_TO_DEF + _str * STR_TO_DEF + vit * VIT_TO_DEF

        # HP 计算
        LEVEL_TO_HP = PropertyConverter.get("LEVEL", "HP")
        STR_TO_HP = PropertyConverter.get("STR", "HP")
        VIT_TO_HP = PropertyConverter.get("VIT", "HP")
        final_hp = GameBaseInitData.BASE_HP + (level - 1) * LEVEL_TO_HP + _str * STR_TO_HP + vit * VIT_TO_HP

        # SP 计算
        LEVEL_TO_SP = PropertyConverter.get("LEVEL", "SP")
        VIT_TO_SP = PropertyConverter.get("VIT", "SP")
        final_sp = GameBaseInitData.BASE_SP + (level - 1) * LEVEL_TO_SP + vit * VIT_TO_SP

        # CRI
        AGI_TO_CRI = PropertyConverter.get("AGI", "CRI")
        final_cri = GameBaseInitData.BASE_CRI + agi * AGI_TO_CRI

        # EVA
        AGI_TO_EVA = PropertyConverter.get(PropNames.AGI, PropNames.EVA)
        final_eva = GameBaseInitData.BASE_EVA + agi * AGI_TO_EVA

        # HIT
        AGI_TO_HIT = PropertyConverter.get(PropNames.AGI, PropNames.HIT)
        final_hit = GameBaseInitData.BASE_HIT + agi * AGI_TO_HIT
