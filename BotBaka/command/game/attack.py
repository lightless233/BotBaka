...
from ...database.models import PlayerModel, PlayerRegainModel
...
"""
    ...
    def process(self, from_group: int, from_qq: int, name: str, command_list: List[str]):
        ...
        # 0. 获取参数
        try:
            _target_qq = target_qq = command_list[2]
        ...
        with transaction.atomic():
            ...
            if attacker_obj.current_hp == 0:
                self.CQApi.send_group_message(from_group, from_qq, "死人是不会攻击的。")
                return
            # 护盾校验，如果攻击者有盾，破盾，如果目标有盾，不能打
            # 破攻击者的盾
            _regain_obj: PlayerRegainModel = PlayerRegainModel.instance.filter(qq=from_qq).first()
            if _regain_obj:
                _regain_obj.shield_time = datetime.datetime.now() - datetime.timedelta(minutes=1)
                _regain_obj.save()
